...
struct DoctorReportView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: HenriiSpacing.lg) {
                    ...
                    VStack(spacing: HenriiSpacing.md) {
                        ShareLink(
                            item: reportText,
                            subject: Text("\(baby.name) — Baby Care Report"),
                            message: Text("Report generated by Henrii")
                        ) {
                            HStack {
    ...
    private func generateReport() -> String {
        ...
        var report = """
        BABY CARE REPORT
        \(baby.name) \u{2014} \(baby.ageDescription)
        Born: \(formatter.string(from: baby.birthDate))
        Report Period: \(formatter.string(from: weekAgo)) \u{2013} \(formatter.string(from: Date()))
        ...
        if !healthEvents.isEmpty {
            ...
            for event in healthEvents {
                report += "\n\u{2022} \(formatter.string(from: event.timestamp)): \(event.summaryText)"
            }
        }
